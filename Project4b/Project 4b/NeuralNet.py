...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        # new weight = old weight + inActs * alpha * delta
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            next = []
            for p in layer:
                next.append(p.sigmoidActivation(output[-1]))
            output.append(next)
        return output
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    # delta[i] = gPrime(in) * sumOfWeights * delta[j]
                    sum = 0
                    for nextNeuronNum in xrange(len(nextLayer)):
                        sum += nextLayer[nextNeuronNum].weights[neuronNum + 1] * deltas[0][nextNeuronNum]
                    delta = gPrime * sum
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for input, output in examplesTest:
        # feedforward to get the NN outputs
        NNoutput = nnet.feedForward(input)[-1]

        # variable to check if the entire rounded list from the NN matches with the known list from the test example
        matchCheck = True

        # round the list of outputs from the output layer of the neuralnet
        # and check if the entire rounded list from the NN matches with the known list from the test example
        for i in xrange(len(output)):
            if round(NNoutput[i]) != output[i]:
                matchCheck = False

        # if match, add to testCorrect
        if matchCheck:
            testCorrect += 1
        # if not, add to testError
        else:
            testError += 1
