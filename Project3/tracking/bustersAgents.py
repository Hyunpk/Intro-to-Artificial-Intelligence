...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostPosition in livingGhostPositionDistributions:
            mostLikelyPos.append(ghostPosition.argMax())

        # find the closest ghost
        closest = mostLikelyPos[0]
        minDist = -1
        for ghostPos in mostLikelyPos:
            if minDist == -1 or self.distancer.getDistance(pacmanPosition, ghostPos) < minDist:
                minDist = self.distancer.getDistance(pacmanPosition, ghostPos)
                closest = ghostPos

        #chooses an action that brings Pacman to closer to the closest ghost
        nextAction = legal[0]
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            if self.distancer.getDistance(successorPosition, closest) < minDist:
                minDist = self.distancer.getDistance(successorPosition, closest)
                nextAction = action
        return nextAction
