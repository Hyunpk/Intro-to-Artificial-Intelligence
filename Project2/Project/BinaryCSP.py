...
"""
	Implementation of queensProblemConstraint
	Satisfied if two queens are not on 1)the same column, 2)the same row,  and 3)the same diagonal
"""
class queensProblemConstraint(BinaryConstraint):
	def isSatisfied(self, value1, value2):
		...
		if value1[1] == value2[1]:
			return False
		if ((int(value1[1]) - int(value1[0])) == (int(value2[1]) - int(value2[0]))) or (-(int(value1[1]) - int(value1[0])) == (int(value2[1]) - int(value2[0]))):
			return False
		return True
...
def consistent(assignment, csp, var, value):
	...
	for const in csp.binaryConstraints:
		if const.affects(var):
			otherVar = const.otherVariable(var)
			# check if the current constraint is satisfied.
			if not const.isSatisfied(assignment.assignedValues[otherVar], value):
				return False
	return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
	"""Question 1"""
	"""YOUR CODE HERE"""
	# check if the assignment is complete and return the complete assignment
	if assignment.isComplete():
		return assignment

	# get the next variable and order values
	var = selectVariableMethod(assignment, csp)
	orderValues = orderValuesMethod(assignment, csp, var)

	# go through order values
	for val in orderValues:
		if consistent(assignment, csp, var, val):
			assignment.assignedValues[var] = val
			updatedAssignment = recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod)
			if updatedAssignment is not None:
				return updatedAssignment
			# clean up the assignment of var if updatedAssignment is None
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	for var in domains:
		if not assignment.isAssigned(var):
			# minimum remaining value heuristic to pick next variable
			rv = len(domains[var])
			# check if the remaining value of the current variable is minimum remaining value
			if minRv > rv:
				minRv = rv
				nextVar = var

			# degree heuristic for breaking ties
			if minRv == rv:
				# compute the degree of the current variable
				degree = 0
				for const in csp.binaryConstraints:
					if const.affects(var):
						if not assignment.isAssigned(const.otherVariable(var)):
							degree += 1
				# compare the computed degree to the maxDegree
				if degree > maxDegree:
					maxDegree = degree
					nextVar = var
	return nextVar
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	"""Question 3"""
	"""YOUR CODE HERE"""
	# helper function that returns the number of constrained values
	def countConstrainedValues(assignment, csp, var, val):
		count = 0
		for const in csp.binaryConstraints:
			if const.affects(var):
				otherVar = const.otherVariable(var)
				for anotherVal in assignment.varDomains[otherVar]:
					if not const.isSatisfied(val, anotherVal):
						count += 1
		return count

	# uses priority queue
	# priority = the number of constrained values
	pQueue = Queue.PriorityQueue()
	for val in values:
		priority = countConstrainedValues(assignment, csp, var, val)
		pQueue.put((priority, val))

	# create the ordered list from the priority queue
	orderedList = []
	while not pQueue.empty():
		orderedList.append(pQueue.get()[1])
	return orderedList
...
def forwardChecking(assignment, csp, var, value):
	...
	"""Question 4"""
	"""YOUR CODE HERE"""
	# loop through binaryConstraints and update inferences
	for const in csp.binaryConstraints:
		if const.affects(var):
			otherVar = const.otherVariable(var)
	...
	# detect inconsistent assignment and reverse inferences
	for var in assignment.assignedValues:
		if len(domains[var]) == 0:
			for inf in inferences:
				domains[inf[0]].add(inf[1])
			return None
	return inferences
	...
	for val in orderValues:
		if consistent(assignment, csp, var, val):
			...
			if inferences is not None:
				updatedAssignment = recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod)
				if updatedAssignment is not None:
					return updatedAssignment
...
def revise(assignment, csp, var1, var2, constraint):
	...
	"""Question 5"""
	"""YOUR CODE HERE"""
	# loop through domain of var2 and update inferences
	...
	# detect inconsistency and return None if inconsistent
	if len(assignment.varDomains[var2]) == len(inferences):
		return None

	# return inferences otherwise
	for inf in inferences:
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	"""Question 5"""
	"""YOUR CODE HERE"""
	# my implementation utilizes Figure 6.3 of page 209 of the Textbook

	# initialize a queue of arcs
	# the item in the list will be in a form of (var, otherVar, constraints)
	queue = Queue.Queue()
	for const in csp.binaryConstraints:
		if const.affects(var):
			otherVar = const.otherVariable(var)
			queue.put((var, otherVar, const))

	# loop through the queue and update inferences
	while not queue.empty():
		arc = queue.get()
		arcVar1 = arc[0]
		arcVar2 = arc[1]
		arcConst = arc[2]
		arcInferences = revise(assignment, csp, arcVar1, arcVar2, arcConst)
		# detect inconsistent assignment
		if arcInferences is None:
			for inf in inferences:
				assignment.varDomains[inf[0]].add(inf[1])
			return None
		# otherwise update the inferences and queue
		else:
			for inf in arcInferences:
				inferences.add(inf)
				for const in csp.binaryConstraints:
					if const.affects(arcVar2) and const.otherVariable(arcVar2) is not arcVar1:
						queue.put((arcVar2, const.otherVariable(arcVar2), const))
	return inferences
...
def AC3(assignment, csp):
	...
	queue = Queue.Queue()
	for const in csp.binaryConstraints:
		queue.put((const.var1, const.var2, const))
		queue.put((const.var2, const.var1, const))
